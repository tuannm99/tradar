...
3. **Schema diff/compare** — **Chốt phạm vi 2026-09-23** (qua `AskUserQuestion`, sau khi khảo sát code): so **2 connection đã lưu bất kỳ** (không cần cùng loại DB) — so 2 schema trong cùng 1 connection để sau. V1 diff trên `SchemaInfo`/`ColumnInfo` hiện có (tên cột, kiểu, PK, FK) — **không** mở rộng `list_schema` với index/constraint/default trước, để khỏi block vào việc sửa cả 5 driver (Postgres/SQLite/Mongo/Elasticsearch/Cassandra); vòng mở rộng `list_schema` (index/constraint/default) để làm round sau nếu thấy cần. UI: **tab riêng mở từ navigator** (chọn connection A + B), không phải overlay kiểu F4 ERD — vì diff vốn cross-connection còn `ErdComponent` (`crates/tradar-query-workbench/src/components/erd.rs`) chỉ với tới schema của tab hiện tại (`engine.schema()` của chính nó), không có đường nào sang connection thứ 2. Output **read-only**: chỉ liệt kê khác biệt (cột thiếu/thừa, kiểu khác) — không sinh DDL để đồng bộ, tách khỏi rủi ro sinh-DDL-sai mà #2 Table designer gặp phải. Việc mới cần làm, theo khảo sát: (a) fetch `list_schema` từ 2 connection song song — chưa có sẵn, `QueryEngine::schema()` (`crates/tradar-query-workbench/src/query_engine.rs`) hiện chỉ cache theo từng connection riêng lẻ; (b) tái dùng `ui::SplitPane`/`panel`/`draw_status_bar` từ `tradar-core::ui` cho layout side-by-side thay vì vẽ chrome mới, theo đúng nguyên tắc "tái dùng widget chung" của dự án.
