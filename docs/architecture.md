...
- **MongoDB**: một parser tối giản cho đúng shape `db.<collection>.<method>(<json-args>)` với chain tuỳ chọn sau `find(...)` — không phải JS engine thật. Hỗ trợ `find` (nhận cả projection ở đối số thứ 2, và chain `.sort()`/`.limit()`/`.skip()`/`.count()`), `findOne`, `countDocuments`, `aggregate`, `insertOne`, `insertMany`, `updateOne`, `updateMany`, `deleteOne`, `deleteMany` (2026-09-10, `docs/backlog/mongo-chaining.md` — trước đó không có chaining/projection/hai lệnh này). Không có `$where`, không có bulk operation hay transaction, không chain được trên method nào khác ngoài `find`; bất cứ gì ngoài shape này trả về lỗi "unsupported query".
