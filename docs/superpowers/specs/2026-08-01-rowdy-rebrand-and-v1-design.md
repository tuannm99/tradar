...
The project currently lives in this repo under the Cargo package name `tradar`, a leftover from an earlier, unrelated concept (an algorithmic trading platform). The current `init.md` product brief describes something different: a terminal-first database exploration and query tool (TUI), in the spirit of LazyGit or k9s but for databases — a unified, keyboard-driven interface for querying, browsing, analyzing, and managing multiple databases (PostgreSQL, MySQL, MariaDB, SQLite, MongoDB, Elasticsearch, with Redis/ClickHouse planned) without switching between native CLI clients, while preserving each database's native query language rather than inventing a custom one.

The codebase itself is still just Cargo scaffolding (`src/main.rs` prints "Hello, world!"). This spec covers renaming the project and defining the scope and architecture for its first real (v1) milestone.

## Goals

1. Rename the project from `tradar` to `rowdy` and update all references.
2. Define a v1 ("walking skeleton") scope: connect to a database, browse its schema, run a query, see results — end to end, keyboard-only — for exactly two drivers (PostgreSQL and SQLite).
3. Establish an architecture with strict isolation between core application logic and per-database driver code, so additional drivers can be added later with minimal change to the core.
4. Produce documentation (`README.md`, `docs/architecture.md`) that captures the product pitch, v1 scope, and architecture, replacing `init.md` as the primary reference.
...
## Rebrand: Tradar → Rowdy

- Rename the Cargo package (`Cargo.toml` `[package] name`) and resulting binary from `tradar` to `rowdy`.
...
- `CLAUDE.md` is updated to reflect the new name and this spec's scope.

Name rationale: "Rowdy" is a direct pun on database *rows*, is a real, easy-to-type CLI word, and has no known collisions with existing developer tools (checked against DBeaver, TablePlus, DataGrip, Beekeeper Studio, pgcli, mycli, litecli, usql, lazysql, FerretDB, BadgerDB, Marmot, Delve, MongoDB Compass — none share the name).

## v1 Scope ("walking skeleton")

Goal: `rowdy` launches, the user picks a saved connection (Postgres or SQLite), browses its schema, runs a query, and sees results — fully keyboard-driven, no mouse required.
