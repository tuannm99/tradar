...
- `tree-sitter` + `tree-sitter-highlight` + `tree-sitter-sequel` — SQL syntax highlighting in the query editor (`tradar-query-workbench`'s `sql_highlight.rs`), Postgres/SQLite only
- `toml` — saved-connections/session file format (`tradar-core` only)
- `directories` — platform-appropriate config path for the saved-connections/session files (`tradar-core` only)
